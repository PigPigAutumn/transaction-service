...
CREATE IF NOT EXISTS INDEX idx_account_time_id ON t_transaction (account, create_time DESC, id DESC);

-- Create an index for global queries
CREATE IF NOT EXISTS INDEX idx_time_id ON t_transaction (create_time DESC, id DESC);

-- Create an index for specifying the user and type
CREATE IF NOT EXISTS INDEX idx_account_type_time_id ON t_transaction (account, type, create_time DESC, id DESC);
